...
def test_network_available():
	...
	while(not connection and iteration < 4) :
			...
			response=urllib2.urlopen('http://blutrokonix.000webhostapp.com/', timeout=3)
			connection = True
...
def thingspeak_uploadMultipleData(data_array):
	...
	if(connected):
		...
		# For testing thinghtronics
		#cmd += ' https://api.thingspeak.com/update?key=XNO1RA7JUS8BEGFX'
		cmd += ' http://blutrokonix.000webhostapp.com/get.php?name=Ttronics&field'+str(temp_fieldnumber)+'='+data_array[temp_iteration]+'&field'+str(temp_fieldnumber+1)+'='+data_array[temp_iteration+2]
		print("ThingSpeak: will issue curl cmd")
		print(cmd)
		args = cmd.split()
		print(args)
